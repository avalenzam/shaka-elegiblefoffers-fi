...
public class OffersBenefitsService implements OfferBenefitsServiceI {
	...
	public ResponseType getOfferBenefitsFi(OffersBenefitsRequestDto offersBenefitsRequestDto)
		...
		try {
			...
			if (productType.contains(Constant.SVA)) {
				...
				ResponseType rt = getOfferAndBenefit(offersBenefitsRequestDto);
				
				List<OfferingType> offerBenefitsList = rt.getOfferings();
				paginationInfo = rt.getPaginationInfo();
		...
		for (CategoryListType categories : rort.getCategories()) {
			...
			for (OfferingTypeOfferType offering : categories.getOfferings()) {
				...
				if (Objects.nonNull(offering)) {
					...
					Boolean planCid = offerFilter.offerFilter(offersBenefitsRequestDto,
							offering.getCatalogItemId());

					if (Boolean.TRUE.equals(planCid)) {

						String vProductOfferingID = offering.getCatalogItemId();
						Map<String, List<OffersProperties>> offersPropertiesMap = cacheOffersPropertiesCharge
								.getOffersProperties();
						List<OffersProperties> offersProperties = offersPropertiesMap
								.get(vProductOfferingID);
						String downloadSpeed = null;
						BigDecimal amount = BigDecimal.valueOf(0);

						for (OfferingTypeOfferType children : offering.getChildren()) {

							if (ProductTypeEnumType.BROADBAND
									.equals(children.getProductType().get(0))) {
								isInternet = Boolean.TRUE;
								List<PricingType> priceList = children.getPlanBoList().get(0)
										.getPriceList();
								if (Boolean.FALSE.equals(priceList.isEmpty())) {

									// TODO VERIFICAR QUE VELOCIODAD Y PRECIO OBTENER, AMDOCS TRAE
									// VARIOS
									for (PricingType price : priceList) {
										...
										offeringTypeList.add(fillOfferingType(offering, children,
												downloadSpeed, offersBenefitsRequestDto,
						...
						if (Boolean.FALSE.equals(isInternet)) {
							...
							offeringTypeList.add(fillOfferingType(offering, null, downloadSpeed,
									offersBenefitsRequestDto, offersProperties, amount,
									categories));
			...
			if (Boolean.FALSE.equals(Util.isEmptyOrNullList(offeringTypeList))) {
				responseType.setPaginationInfo(fillPaginationInfo(categories.getPaginationInfo(),
						isInternet, downloadSpeedSum,
	...
	private OfferingType fillOfferingType(OfferingTypeOfferType offering,
			OfferingTypeOfferType childrenInternet, String downloadSpeed,
			OffersBenefitsRequestDto offersBenefitsRequestDto,
			List<OffersProperties> offersProperties, BigDecimal amount,
		...
		AditionalSvaResponse aditionalSvaResponse = aditionalSva.getAditionalSva(vProductOfferingID,
				downloadSpeed, offersBenefitsRequestDto, offersProperties);
		...
		for (OfferingTypeOfferType children : offering.getChildren()) {
			...
			} else if (ProductTypeEnumType.CABLE_TV.toString()
					.equalsIgnoreCase(childreProductType.toString())) {
				productSpecification.setProductType(ProductTypeEnum.CABLETV);
			} else {
				productSpecification.setProductType(
						ProductTypeEnum.fromValue(childreProductType.toString().toLowerCase()));
			}
			
			productCharacteristicsList.add(fillProductCharacteristics(null,
					Constant.PRODUCTOFFERINGPRODUCTSPECID, ValueTypeEnum.STRINGWRAPPER,
					children.getProductOfferingProductSpecID()));
			...
			productCharacteristicsList
					.add(fillProductCharacteristics(null, Constant.PARENT_CATALOG_ITEM_ID,
							ValueTypeEnum.STRINGWRAPPER, children.getParentCatalogItemID()));
			productCharacteristicsList
					.add(fillProductCharacteristics(null, Constant.PARENT_CATALOG_ITEM_NAME,
							ValueTypeEnum.STRINGWRAPPER, children.getParentCatalogItemName()));
			productCharacteristicsList
					.add(fillProductCharacteristics(null, Constant.PARENT_CURRENT_STATUS,
							ValueTypeEnum.STRINGWRAPPER, children.getParentCurrentStatus()));
			productCharacteristicsList
					.add(fillProductCharacteristics(null, Constant.PARENT_ASSIGNED_ID,
			...
			productCharacteristicsList.add(fillProductCharacteristics(null,
					Constant.TOP_RECOMMENDED, ValueTypeEnum.STRINGWRAPPER,
					Objects.nonNull(children.isTopRecommended())
							? Boolean.toString(children.isTopRecommended())
							: null));
			productCharacteristicsList.add(fillProductCharacteristics(null,
					Constant.COMPATIBLE_WITH_DEVICE, ValueTypeEnum.STRINGWRAPPER,
					Optional.ofNullable(children.getCompatibleWithDevice()).map(x -> x)
							.orElse(null)));
			productCharacteristicsList
					.add(fillProductCharacteristics(null, Constant.MIN_NUM_SUBSCRIBERS,
							ValueTypeEnum.STRINGWRAPPER, children.getMinNumOfSubscribers()));
			productCharacteristicsList
					.add(fillProductCharacteristics(null, Constant.NUM_SUBSCRIBERS,
			...
			productCharacteristicsList.add(fillProductCharacteristics(null, Constant.IMAGE,
					ValueTypeEnum.STRINGWRAPPER, children.getImage()));
			...
			children.getAdditionalData().forEach(additionalData -> {
				...
				if (Boolean.FALSE.equals(StringUtil.isNullOrEmpty(additionalData.getValue()))) {
					productCharacteristicsList
							.add(fillProductCharacteristics(null, additionalData.getKey(),
			...
			if (!children.getPriceDetails().isEmpty()) {
				for (PriceDetailsType priceDetail : children.getPriceDetails()) {
					...
					if (PriceTypeProdAltType.RECURRING_ALLOWANCE
							.equals(priceDetail.getPriceType())) {
						productPrice.setPriceType(PriceTypeEnum.RECURRING);
					} else if (PriceTypeProdAltType.ONE_TIME_ALLOWANCE
							.equals(priceDetail.getPriceType())) {
			...
			for (PlanBODetailsType planBo : children.getPlanBoList()) {
				...
				if (ProductTypeEnumType.BROADBAND.equals(productType)) {
					...
					planBo.getPriceList().forEach(priceList -> {
						...
						productPricePo.setName(Constant.PRECIO_VELOCIDAD
								+ " " + priceList.getDownloadSpeed() + " " + Constant.MBPS);
						...
						productPricePo
								.setPriceWithTax(fillMoneyType(priceList.getPrice().getAmount(),
						...
						additionalDataPlanBoList.add(
								fillAdittionalData(Constant.TECNOLOGY, priceList.getTechnology()));
						additionalDataPlanBoList.add(fillAdittionalData(Constant.DOWNLOAD_SPEED,
								priceList.getDownloadSpeed()));

						productPricePo.setAdditionalData(additionalDataPlanBoList);
						productPricePo
								.setProductSpecContainmentID(planBo.getProductSpecContainmentID());
						productPricePo.setPricePlanSpecContainmentID(
								planBo.getPricePlanSpecContainmentID());
					...
					productPricePo
							.setProductSpecContainmentID(planBo.getProductSpecContainmentID());
					productPricePo
							.setPricePlanSpecContainmentID(planBo.getPricePlanSpecContainmentID());
					...
					for (PriceDetailsType priceDetail : planBo.getPriceDetails()) {
						...
						productPricePo.setPrice(fillMoneyType(priceDetail.getPrice().getAmount(),
								priceDetail.getPrice().getUnits(), Boolean.FALSE));
						productPricePo
								.setMinPrice(fillMoneyType(priceDetail.getMinPrice().getAmount(),
										priceDetail.getMinPrice().getUnits(), Boolean.FALSE));
						productPricePo
								.setMaxPrice(fillMoneyType(priceDetail.getMaxPrice().getAmount(),
										priceDetail.getMaxPrice().getUnits(), Boolean.FALSE));
						productPricePo.setOriginalAmount(
								fillMoneyType(priceDetail.getOriginalAmount().getAmount(),
										priceDetail.getOriginalAmount().getUnits(), Boolean.FALSE));
						productPricePo.setOriginalTaxAmount(
								fillMoneyType(priceDetail.getOriginalAmount().getAmount(),
										priceDetail.getOriginalAmount().getUnits(), Boolean.TRUE));
					...
					planBo.getPlanInfo().forEach(planInfo -> {
						...
						additionalDataPlanBoList
								.add(fillAdittionalData(planInfo.getKey(), planInfo.getValue()));
						productPricePo.setAdditionalData(additionalDataPlanBoList);
					});
		...
		for (PriceDetailsType priceDetail : offering.getPriceDetails()) {
			...
			if (ProductTypeEnumType.BROADBAND.equals(productType)) {
				productOfferingPrice.setPrice(
						fillMoneyType(priceDetail.getOriginalAmount().getAmount().add(amount),
								priceDetail.getOriginalAmount().getUnits(), Boolean.FALSE));
				productOfferingPrice.setPriceWithTax(
						fillMoneyType(priceDetail.getOriginalAmount().getAmount().add(amount),
								priceDetail.getOriginalAmount().getUnits(), Boolean.TRUE));
				productOfferingPrice.setOriginalAmount(
						fillMoneyType(priceDetail.getOriginalAmount().getAmount().add(amount),
								priceDetail.getOriginalAmount().getUnits(), Boolean.FALSE));
				productOfferingPrice
						.setOriginalTaxAmount(fillMoneyType(
								priceDetail.getOriginalAmount().getAmount()
										.add(Objects.nonNull(amount) ? Util.addIgv(amount)
												: BigDecimal.ZERO),
								priceDetail.getOriginalAmount().getUnits(), Boolean.FALSE));
			} else {
				productOfferingPrice
						.setPrice(fillMoneyType(priceDetail.getOriginalAmount().getAmount(),
								priceDetail.getOriginalAmount().getUnits(), Boolean.FALSE));
				productOfferingPrice
						.setPriceWithTax(fillMoneyType(priceDetail.getOriginalAmount().getAmount(),
								priceDetail.getOriginalAmount().getUnits(), Boolean.TRUE));
				productOfferingPrice.setOriginalAmount(
						fillMoneyType(priceDetail.getOriginalAmount().getAmount(),
								priceDetail.getOriginalAmount().getUnits(), Boolean.FALSE));
				productOfferingPrice.setOriginalTaxAmount(
						fillMoneyType(priceDetail.getOriginalAmount().getAmount(),
								priceDetail.getOriginalAmount().getUnits(), Boolean.TRUE));
			}
		...
		additionalDataList
				.add(fillAdittionalData(Constant.DISPLAY_NAME, offering.getDisplayName()));
		additionalDataList.add(fillAdittionalData(Constant.RELATION_ID, offering.getRelationId()));
		additionalDataList
				.add(fillAdittionalData(Constant.ID_ASSIGNED_ITEM, offering.getCorrelationId()));
		additionalDataList.add(fillAdittionalData(Constant.PARENT_CATALOG_ITEM_ID,
				offering.getParentCatalogItemID()));
		additionalDataList.add(fillAdittionalData(Constant.PARENT_CATALOG_ITEM_NAME,
				offering.getParentCatalogItemName()));
		additionalDataList.add(fillAdittionalData(Constant.PARENT_CURRENT_STATUS,
				offering.getParentCurrentStatus()));
		additionalDataList.add(
				fillAdittionalData(Constant.PARENT_ASSIGNED_ID, offering.getParentAssignedID()));
		additionalDataList.add(fillAdittionalData(Constant.TOP_RECOMMENDED,
				Objects.nonNull(offering.isTopRecommended())
						? Boolean.toString(offering.isTopRecommended())
						: null));
		...
		additionalDataList.add(fillAdittionalData(Constant.COMPATIBLE_WITH_DEVICE,
				offering.getCompatibleWithDevice()));
		additionalDataList.add(fillAdittionalData(Constant.MIN_NUM_SUBSCRIBERS,
				offering.getMinNumOfSubscribers()));
		additionalDataList
				.add(fillAdittionalData(Constant.NUM_SUBSCRIBERS, offering.getNumOfSubscribers()));
		...
		UpfrontFijaResponse upfrontFijaResponse = upfrontFija.getUpfrontFija(
				offersBenefitsRequestDto, aditionalSvaResponse.getOfferData().getLob());
		...
		additionalDataList.add(
				fillAdittionalData(Constant.INSTALLATION_FEE_BO, upfrontFijaResponse.getCidBo()));
		additionalDataList.add(fillAdittionalData(Constant.PRODUCT_FOR_INST_FEE,
				upfrontFijaResponse.getProductForInstFee()));
		...
		// TODO BENEFICIOS
		offeringType.setBenefits(
				getBenefits(offersBenefitsRequestDto, vProductOfferingID, downloadSpeed));
		...
		offeringType.setAdditionalData(additionalDataList.stream().filter(x -> Objects.nonNull(x))
		...
		if (StringUtil.isNullOrEmpty(productPublicId)
				&& StringUtil.isNullOrEmpty(productTypeRequest)) {

			List<ProductInventoryResponseDto> productInventory = parqueUnificadoConnection
					.callRestService(productPublicId, productTypeRequest);
	 ...
	 */
	private ComposingProductType svsincluding(AditionalSvaResponse aditionalSvaResponse,
			ProductTypeEnumType productType) {
		...
		if (ProductTypeEnumType.SH_EQ.equals(productType)) {
			...
			subProductsList.add(fillSubProducts(modem.getEquipmentCid(), modem.getNameComp(),
					ProductTypeEnum.DEVICE));

			if (Boolean.TRUE.equals(aditionalSvaResponse.getOfferData().getFlagModemPremium())) {

				productCharacteristicsList.add(fillProductCharacteristics(
						Constant.EQUIPMENT_SUB_TYPE, Constant.SUB_TIPO_EQUIPO,
		...
		if (ProductTypeEnumType.CABLE_TV.equals(productType)) {
			...
			aditionalSvaResponse.getDecos().forEach(decos -> {
				subProductsList.add(
						fillSubProducts(Constant.STB, decos.getCaption(), ProductTypeEnum.DEVICE));
				productCharacteristicsList.add(
						fillProductCharacteristics(Constant.STB_TYPE, Constant.TIPO_DECODIFICADOR,
		...
		if (!(Constant.NULL.equalsIgnoreCase(aditionalSvaResponse.getOfferData().getDefSpsBo())
			...
			aditionalSvaResponse.getChannelBlock().forEach(channelBlock -> {
				...
				subProductsList.add(fillSubProducts(channelBlock.getFdIdParent(),
						channelBlock.getFdNameParent(), ProductTypeEnum.SVA));

				productCharacteristicsList.add(
						fillProductCharacteristics(channelBlock.getCidBo(), Constant.BILLING_OFFER,
								ValueTypeEnum.STRINGWRAPPER, channelBlock.getDescriptionText()));

				productCharacteristicsList
						.add(fillProductCharacteristics(channelBlock.getDefSpsId(), Constant.SPSID,
		...
		aditionalSvaResponse.getOtherSvas().forEach(otherSvas -> {
			...
			if (ProductTypeEnumType.BROADBAND.equals(productType)
					&& Constant.MCAFEE.equals(otherSvas.getParentId())
					|| ProductTypeEnumType.LANDLINE.equals(productType)
							&& Constant.MULTIDESTINO.equals(otherSvas.getParentId())) {

				subProductsList.add(fillSubProducts(otherSvas.getParentId(),
						otherSvas.getNameComp(), ProductTypeEnum.SVA));

				productCharacteristicsList.add(
						fillProductCharacteristics(otherSvas.getCidBo(), Constant.BILLING_OFFER,
	 ...
	 */
	private List<BenefitType> getBenefits(OffersBenefitsRequestDto offersBenefitsRequestDto,
			String vProductOfferingID, String downloadSpeed) {

		List<BenefitsResponse> benefitList = benefit.getBenefitDiscount(offersBenefitsRequestDto,
				vProductOfferingID, downloadSpeed);
		...
		benefitList.forEach(benefits -> {
			...
			characteristicsList.add(fillCharacteristics(Constant.SPS_ID,
					benefits.getBenefits().getBenefitThemePackSpsCid()));
			characteristicsList.add(fillCharacteristics(Constant.SPS_NAME, benefits.getSpsName()));
			characteristicsList.add(fillCharacteristics(Constant.BO_ID,
					benefits.getBenefits().getBenefitBillingOfferCid()));
			characteristicsList.add(fillCharacteristics(Constant.BO_NAME, benefits.getNameBo()));
			characteristicsList.add(
					fillCharacteristics(Constant.DURATION, benefits.getBenefits().getDuration()));
			characteristicsList.add(
					fillCharacteristics(Constant.NIGHT_ID, benefits.getBenefits().getNightInd()));
			...
			if (Constant.MONETARY.equalsIgnoreCase(benefits.getValueAbpType())) {
				...
				priceBenefit.setPriceWithTax(
						fillMoneyType(value, Constant.PERUVIAN_COIN, Boolean.TRUE));
	...
	private List<OfferingType> getSva(OffersBenefitsRequestDto offersBenefitsRequestDto) {
		...
		List<String> productOfferingCatalogIdList = Arrays
				.asList(offersBenefitsRequestDto.getProductOfferingCatalogId().split(","));

		productOfferingCatalogIdList.forEach(productOfferingCatalogId -> {

			Map<String, List<OffersProperties>> offersPropertiesMap = cacheOffersPropertiesCharge
					.getOffersProperties();
			List<OffersProperties> propertyValueList = offersPropertiesMap
					.get(productOfferingCatalogId);
			...
			if (Constant.YES.equalsIgnoreCase(retention)
					&& Boolean.TRUE.equals(offersBenefitsRequestDto.getIsRetention())) {

				svaResponseList = sva.getSvaTypeRetention(offersBenefitsRequestDto, flagRetention,
						propertyValueList, productOfferingCatalogId);
					...
					svaResponseList.forEach(svaResponse -> {
						...
						svaResponse.getBillingOffer().forEach(billingOffer -> {
							...
							productSpecification
									.setName(billingOffer.getBillingOffer().getNameChild());

							productSpecification.setProductType(ProductTypeEnum.SVA);
							productSpecification.setPeriodDuration(
									billingOffer.getBillingOffer().getDurationValue());

							productCharacteristicsList.add(fillProductCharacteristics(null,
									Constant.SPSID, ValueTypeEnum.STRINGWRAPPER,
									billingOffer.getSpsIdAndName().getParentId()));
							productCharacteristicsList.add(fillProductCharacteristics(null,
									Constant.NOMBRE_SPS, ValueTypeEnum.STRINGWRAPPER,
									billingOffer.getSpsIdAndName().getNameParent()));

							refinedProductType
									.setProductCharacteristics(productCharacteristicsList);
							productSpecification.setRefinedProduct(refinedProductType);

							productPrice.setName(Constant.PRECIO_SVA);
							productPrice.setProductSpecContainmentID(
									billingOffer.getBillingOffer().getRelationId());
							productPrice
									.setPricePlanSpecContainmentID(billingOffer.getRelationId());
							productPriceList.add(productPrice);
							productSpecification.setProductPrice(productPriceList);
					...
					svaResponseList.forEach(svaResponse -> {
						...
						svaResponse.getBillingOffer().forEach(billingOffer -> {
							...
							productSpecification
									.setName(billingOffer.getBillingOffer().getNameChild());
							productSpecification
									.setProductType(ComposingProductType.ProductTypeEnum.SVA);

							if (Constant.STB.equals(svaResponse.getIdComponent())) {
								productSpecification
										.setMaxCardinality(billingOffer.getMaxSTBsallowed());
							}
							...
							productPrice.setPrice(fillMoneyType(billingOffer.getAmount(),
									Constant.PERUVIAN_COIN, Boolean.FALSE));
							productPrice.setProductSpecContainmentID(
									billingOffer.getBillingOffer().getRelationId());
							productPrice
									.setPricePlanSpecContainmentID(billingOffer.getRelationId());

							productPrice.setTaxAmount(fillMoneyType(billingOffer.getAmount(),
									Constant.PERUVIAN_COIN, Boolean.TRUE));
							...
							if (Objects.nonNull(billingOffer.getVasBenefits())) {
								...
								benefitType.setId(
										billingOffer.getVasBenefits().getBenefitComponentCid());
								benefitType.setName(billingOffer.getNameComp());
								benefitType
										.setDownloadSpeed(billingOffer.getVasBenefits().getSpeed());

								characteristicsList.add(fillCharacteristics(Constant.SPS_ID,
										billingOffer.getVasBenefits().getBenefitThemePackSpsCid()));
								characteristicsList.add(fillCharacteristics(Constant.SPS_NAME,
										billingOffer.getParentName()));
								characteristicsList.add(fillCharacteristics(Constant.BO_CODE,
										billingOffer.getVasBenefits().getBenefitBillingOfferCid()));
								characteristicsList.add(fillCharacteristics(Constant.BO_NAME,
										billingOffer.getNameBo()));
								characteristicsList.add(fillCharacteristics(Constant.DURATION,
										billingOffer.getVasBenefits().getDuration()));
								benefitType.setCharacteristics(characteristicsList.stream()
										.filter(x -> Boolean.FALSE
												.equals(StringUtil.isNullOrEmpty(x.getValue())))
										.collect(Collectors.toList()));
	 ...
	 */
	private ProductSpecCharacteristicType fillProductCharacteristics(String id, String name,
			ValueTypeEnum enumValue, String value) {
		...
		productCharacteristics
				.setProductSpecCharacteristicValue(productSpecCharacteristicValueTypeList);
	...
	private ComposingProductType fillSubProducts(String id, String name,
	...
	private CategoryTreeType fillCategory(CategoryTreeTypeType categories) {
		...
		subcategories.setId(Optional.ofNullable(categories.getSubcategories())
				.map(CategoryTreeTypeType::getId).orElse(null));
		subcategories.setHref(Optional.ofNullable(categories.getSubcategories())
				.map(CategoryTreeTypeType::getHref).orElse(null));
		subcategories.setName(Optional.ofNullable(categories.getSubcategories())
				.map(CategoryTreeTypeType::getName).orElse(null));
	 ...
	 */
	private PaginationInfoType fillPaginationInfo(PagingInfoOutputType paginationInfo,
			Boolean isInternet, Integer downloadSpeed, Integer page) {
		...
		if (Boolean.TRUE.equals(isInternet)) {
			...
			totalPages = paginationInfo.getTotalResultsInCategory().add(speed)
					.divide(paginationInfo.getItemsPerCategory(), 0, RoundingMode.HALF_UP);
			...
			totalPages = paginationInfo.getTotalResultsInCategory()
	...
	private List<ProductInstanceRefType> fillCompatibleProducts(
			List<ProductInventoryResponseDto> productInventory) {
		...
		compatibleProducts.setProductType(
				(com.telefonica.eof.generated.model.ProductInstanceRefType.ProductTypeEnum
